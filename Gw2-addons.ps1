...
{
    "You are not running this from an actual Guild Wars 2 Directory!"
    Throw "You are not running this from an actual Guild Wars 2 Directory!"
    return # in case you have some weird ErrorAction defaults
}

# check game runnning
if((get-process *gw* | ?{ $_.imagepath -eq (get-item "GW2-64.exe").fullname}).count -gt 0)
{
    write-warning "The game is running! Closing the game is recommended before patching - but you can go on at your own risk!"
    write-warning "Press any key to continue, or CTRL+C to abort"
    pause
}
